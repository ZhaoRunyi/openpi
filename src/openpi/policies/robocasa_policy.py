...
@dataclasses.dataclass(frozen=True)
class Robocasa2ViewsInputs(transforms.PromptFromLeRobotTask):
    """
    This class is used to converted the lerobo-formartted robocasa dataset, collected by human in OpenDrawer environment and do the 'open the right drawer' task
    """
...
@dataclasses.dataclass(frozen=True)
class RobocasaInputs(transforms.PromptFromLeRobotTask):
    """
    This class is used to converted the lerobo-formartted robocasa dataset, collected by human in OpenDrawer environment and do the 'open the right drawer' task
    """
    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            ...
            "image_mask": {
                ...
                "left_wrist_0_rgb": np.True_,
                # We only mask padding images for pi0 model, not pi0-FAST. Do not change this for your own dataset.
                "right_wrist_0_rgb": np.True_ # TODO: set to True although openpi ask us not to, but RoboTwin did so. 
            },
            "task_index": data["task_index"]
        }
