...
def main(config_name: str, base_dir: str | None = None, max_frames: int | None = None):
    ...
    if base_dir is None:
        ...
        base_dir = os.path.join(os.environ.get('HF_LEROBOT_HOME', '/workspace/data'), data_config.repo_id)
    ...
    for file_path in tqdm(parquet_files, desc="Processing files"):
        try:
            # 修复1：仅读取需要的列，极大减少内存占用
            df = pd.read_parquet(file_path, columns=[state_col, action_col])
        ...
        try:
            # 修复2：向量化处理数据，放弃慢速的 for 循环和 iloc
            states_batch = np.stack(df[state_col].values)
            actions_batch = np.stack(df[action_col].values)
            
            # 批量进行 padding 和过滤
            states_batch = process_state(states_batch, action_dim)
            actions_batch = process_actions(actions_batch, action_dim)
            
            collected_data["state"].append(states_batch)
            collected_data["actions"].append(actions_batch)
